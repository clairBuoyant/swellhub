...
**swellhub is single-context**: one `CONTEXT.md` + `docs/adr/` at the repo root, shared by
the Go backend and the `web/` frontend (they speak one ubiquitous language and share
...
```
/
├── CONTEXT.md            ← shared Spot / swell / NOAA glossary
├── docs/adr/             ← architectural decision records
│   └── 0001-*.md
├── internal/             ← Go backend (spot, store, http, …)
├── pkg/                  ← reusable Go packages (noaa, baseplate, …)
└── web/                  ← React/Vite frontend (consumes generated proto clients)
```
...
> _Contradicts ADR-0003 (single-store write model) — but worth reopening because…_
